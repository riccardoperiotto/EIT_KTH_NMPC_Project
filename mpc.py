...
class MPC(object):
    ...
    def __init__(self, model, dynamics,
        ...
        self.x_sp = None
        self.u_prec = np.zeros((6,1))

        # Cost function weights
        Q, R, P, kf, Q_kf, R_kf, C_kf = self.load_params(param, tuning_file)
        ...
        for t in range(self.Nt):
            ...
            x_t = opt_var['x', t]
            if self.trajectory_tracking:
                # obtain the desired step in the reference trajectory
                x_r = x_ref[self.Nx*t : self.Nx*t + self.Nx]
            else:
                x_r = x_ref
            u_t = opt_var['u', t]
    ...
    def load_params(self, param, tuning_file=None):
        ...
        with open(f_path, 'r') as stream:
            ...
            # Get KF parameters
            kf = parameters[param]['kf'] 
            Q_kf_diag = np.asarray(parameters[param]['Q_kf'])
            R_kf_diag = np.asarray(parameters[param]['R_kf'])
            C_kf_values = np.asarray(parameters[param]['C_kf'])
            Q_kf = np.diag(Q_kf_diag)
            R_kf = np.diag(R_kf_diag)
            C_kf = np.zeros((3,6))
            C_kf[0,0] = C_kf_values[0]
            C_kf[1,1] = C_kf_values[1]
            C_kf[2,2] = C_kf_values[2]
            # C_kf[3,6] = C_kf_values[3]
            # C_kf[4,7] = C_kf_values[4]
            # C_kf[5,8] = C_kf_values[5]
            # C_kf[6,9] = C_kf_values[6]

            return Q, R, P, kf, Q_kf, R_kf, C_kf
    ...
    def mpc_controller(self, x0, t):
        ...
        if self.trajectory_tracking and self.kf: 
            # Linearize the dynamic around the new target pB
            Ad, Bd = self.model.linearized_discrete_dynamics(x_traj[:, 0].reshape(13,1), None)

            # Update the Kalman Filter
            x_kf = self.update_kf(Ad, Bd, x0)

            # Solve the MPC problem
            _, u_pred, step_time = self.solve_mpc(x_kf)
        else:
            # Solve the MPC problem
            _, u_pred, step_time = self.solve_mpc(x0)

        self.u_prec = np.asarray(u_pred[0]).reshape(6,1)
       
        # Calculate error to first state
        error = self.calculate_error(x0, self.x_sp[0:13])

        return u_pred[0], error, step_time
    ...
    def update_kf(self, Ad, Bd, x0):

        x0 = np.array(x0).reshape(self.Nx,1)
        
        # Set filter parameters
        # self.kf_estimator.F = Ad
        # self.kf_estimator.B = Bd
        self.kf_estimator.F = Ad[0:6, 0:6].reshape(6, 6)
        self.kf_estimator.B = Bd[0:6, 0:3].reshape(6, 3)
        
        # Set initial estimation
        self.kf_estimator.x = x0[0:6].reshape(6, 1)

        # Estimate the linear  velocity from noisy position measurements
        self.kf_estimator.predict(self.u_prec[0:3].reshape(3,1))

        # Get the measurement
        y = np.dot(self.C_kf, x0[0:6].reshape(6, 1)) # TODO: eventually add noise
        self.kf_estimator.update(y)
        x_kf = self.kf_estimator.x

        x_kf = np.vstack((x_kf, x0[6:].reshape(7, 1))).reshape(13, 1)

        return x_kf.reshape(13,1)
